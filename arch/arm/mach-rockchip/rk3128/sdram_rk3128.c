...
const struct rk3128_ddr_timing ddr_timing = {0x18c,
	...
	{{0x420, 0x42, 0x0, 0x0}, 0x01, 0x60, 0x5},
	{0x24717315} };
...
enum {
	/*
	 * DDRPHY_REG00
	 * Channel-enable bits are not mentioned in the rk3128 TRM, but
	 * used in the binary loader and also mentioned in the rv1108 TRM
	 * so it looks like the rk3128 trm may be wrong.
	 */
	CH_B_HIGH_8BIT_ENABLE			= BIT(7),
	CH_B_LOW_8BIT_ENABLE			= BIT(6),
	CH_A_HIGH_8BIT_ENABLE			= BIT(5),
	CH_A_LOW_8BIT_ENABLE			= BIT(4),
	SOFT_RESET_MASK				= 3,
	SOFT_RESET_SHIFT			= 2,

	/* DDRPHY_REG01 */
	PHY_WORK_MODE_MASK			= 3,
	PHY_WORK_MODE_DDR3			= 0 << 0,
	PHY_WORK_MODE_DDR2			= 1 << 0,
	PHY_WORK_MODE_LPDDR2			= 2 << 0,
...
//	RIGHT_CHN_RX_DQS_DELAY_TAP_SHIFT	= 0,
	...
	/* DDRPHY_REG 26, 36, 46, 56 for Channels A+B (left+right) */
	WRITE_DQ_DLL_PHASE_180DEG_DELAY		= BIT(4),
	WRITE_DQ_DLL_ENABLE			= BIT(3),

	/* DDRPHY_REGFF */
//	CAL_DONE_MASK				= 3,
	CH_B_HIGH_8BIT_CAL_DONE			= 1 << 3,
	CH_B_LOW_8BIT_CAL_DONE			= 1 << 2,
	CH_A_HIGH_8BIT_CAL_DONE			= 1 << 1,
	CH_A_LOW_8BIT_CAL_DONE			= 1 << 0,
};
...
/* GRF_SOC_CON0 */
#define MOBILE_DDR_SEL		BIT(7)

/* GRF_SOC_CON2 */
#define MSCH4_MAINDDR3		BIT(7)
...
enum {
	...
	MSCH4_MAIN_PARTIALPOP_SHIFT = 13,
	MSCH4_MAIN_PARTIALPOP_MASK = 1,

	MOBILE_DDR_SEL_SHIFT	= 7,
	MOBILE_DDR_SEL_MASK	= 1,
	MOBILE_DDR_SEL_DDR23	= 0,
	MOBILE_DDR_SEL_LPDDR23	= 1,
};
...
void phy_pctrl_reset(struct rk3128_sdram_priv *priv)
{
	...
	clrsetbits_le32(&ddr_phy->ddrphy_reg00,
...
*/
    //ddrphy_reg13: cmd dll in bypass 180deg delay
//    *(int32_t *)0x2000a04c = *(int32_t *)0x2000a04c | 16;
    //ddrphy_reg14: clear bit 3: ck dll bypass no delay
//    *(int32_t *)0x2000a050 = *(int32_t *)0x2000a050 & -9;
    //ddrphy_reg26: left channel a dq dll phase 180deg delay
//    int32_t * v2 = (int32_t *)(v1 + 152); // 0x1008090e
//    *v2 = *v2 | 16;
    //ddrphy_reg27: left channel a dqs dll phase no delay
//    int32_t * v3 = (int32_t *)(v1 + 156); // 0x1008091a
//    *v3 = *v3 & -9;
    //ddrphy_reg37: right channel a, as above but wrong offset?
//    int32_t * v4 = (int32_t *)(v1 + 216); // 0x10080926
//    *v4 = *v4 | 16;
    //ddrphy_reg38: right channel a, as above but wrong offset?
//    int32_t * v5 = (int32_t *)(v1 + 220); // 0x10080932
//    *v5 = *v5 & -9;
    //ddrphy_reg46: left channel b, dq dll phase 180deg delay
//    int32_t * v6 = (int32_t *)(v1 + 280); // 0x1008093e
//    *v6 = *v6 | 16;
    //ddrphy_reg47: left channel b, dqs dll phase no delay
//    int32_t * v7 = (int32_t *)(v1 + 284); // 0x1008094a
//    *v7 = *v7 & -9;
...
//FIXME
static void data_training(struct rk3128_sdram_priv *priv, int num_ch)
{
	struct rk3128_ddr_phy *ddr_phy = priv->phy;
	struct rk3128_ddr_pctl *pctl = priv->pctl;
	u32 value;
	...
	setbits_le32(&ddr_phy->ddrphy_reg02, DQS_SQU_CAL_START);

	rockchip_udelay(1);
//regff
/*	while ((readl(&ddr_phy->ddrphy_regff) & CAL_DONE_MASK) !=
		(HIGH_8BIT_CAL_DONE | LOW_8BIT_CAL_DONE)) {
		;
	} */

	clrbits_le32(&ddr_phy->ddrphy_reg02, DQS_SQU_CAL_START);
...
//FIXME: write 500 to trsth?
	...
	writel(RK_CLRBITS(MOBILE_DDR_SEL), &priv->grf->soc_con0);
...
static void phy_cfg(struct rk3128_sdram_priv *priv)
{
	...
	writel(PHY_WORK_MODE_DDR3, &ddr_phy->ddrphy_reg01);

	writel(ddr_timing.phy_timing.cl_al, &ddr_phy->ddrphy_reg0b);
	writel(ddr_timing.phy_timing.cwl, &ddr_phy->ddrphy_reg0c);

	writel(PHY_DRV_ODT_SET(PHY_RON_44OHM), &ddr_phy->ddrphy_reg11);
	writel(PHY_DRV_ODT_SET(PHY_RON_44OHM), &ddr_phy->ddrphy_reg16);
	writel(PHY_DRV_ODT_SET(PHY_RON_44OHM), &ddr_phy->ddrphy_reg20);
	writel(PHY_DRV_ODT_SET(PHY_RON_44OHM), &ddr_phy->ddrphy_reg30);
	writel(PHY_DRV_ODT_SET(PHY_RON_44OHM), &ddr_phy->ddrphy_reg40);
	writel(PHY_DRV_ODT_SET(PHY_RON_44OHM), &ddr_phy->ddrphy_reg50);

	writel(PHY_DRV_ODT_SET(PHY_RTT_216OHM), &ddr_phy->ddrphy_reg21);
	writel(PHY_DRV_ODT_SET(PHY_RTT_216OHM), &ddr_phy->ddrphy_reg31);
	writel(PHY_DRV_ODT_SET(PHY_RTT_216OHM), &ddr_phy->ddrphy_reg41);
	writel(PHY_DRV_ODT_SET(PHY_RTT_216OHM), &ddr_phy->ddrphy_reg51);
}

/* bw: 2: 32bit, 1:16bit */
//ok
static void set_bw(struct rk3128_sdram_priv *priv, u32 bw)
{
	struct rk3128_ddr_pctl *pctl = priv->pctl;
	struct rk3128_ddr_phy *ddr_phy = priv->phy;
	struct rk3128_grf *grf = priv->grf;

	if (bw == 1) {
		setbits_le32(&pctl->ppcfg, 1);
		clrbits_le32(&ddr_phy->ddrphy_reg00, CH_B_HIGH_8BIT_ENABLE | CH_B_LOW_8BIT_ENABLE);
		writel(RK_SETBITS(DDR_16BIT_EN), &grf->soc_con0);
		clrbits_le32(&ddr_phy->ddrphy_reg46, WRITE_DQ_DLL_ENABLE);
		clrbits_le32(&ddr_phy->ddrphy_reg56, WRITE_DQ_DLL_ENABLE);
	} else {
		clrbits_le32(&pctl->ppcfg, 1);
		setbits_le32(&ddr_phy->ddrphy_reg00, CH_B_HIGH_8BIT_ENABLE | CH_B_LOW_8BIT_ENABLE);
		writel(RK_CLRBITS(DDR_16BIT_EN), &grf->soc_con0);
		setbits_le32(&ddr_phy->ddrphy_reg46, WRITE_DQ_DLL_ENABLE);
		setbits_le32(&ddr_phy->ddrphy_reg56, WRITE_DQ_DLL_ENABLE);
	}
}
...
void sdram_init(void)
{
	...
	set_bw(&sdram_priv, sdram_priv.ddr_config.bw);
