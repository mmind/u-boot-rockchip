...
enum {
	...
	/* CLKSEL1: cpu aclk pll sel: arm or general */
	CPU_ACLK_SEL_PLL_SHIFT	= 5,
	CPU_ACLK_SEL_PLL_MASK	= 1,
	CPU_ACLK_SEL_APLL	= 0,
	CPU_ACLK_SEL_GPLL,

	/* cpu aclk div: cpu_aclk = cpu_aclk_src_clk / div */
	CPU_ACLK_DIV_SHIFT	= 0,
	CPU_ACLK_DIV_MASK	= 0x1f,

	/* cpu pclk div: aclk_cpu: pclk_cpu = 1:1 or 2:1 or 4:1 */
	CPU_PCLK_DIV_SHIFT	= 12,
	CPU_PCLK_DIV_MASK	= 3,

	/* cpu hclk div: aclk_bus: hclk_bus = 1:1 or 2:1 or 4:1 */
	CPU_HCLK_DIV_SHIFT	= 8,
	CPU_HCLK_DIV_MASK	= 3,

	/* cpu hclk div: hclk_bus: pclk for uart0,uart1 = 1:1 or 2:1 or 4:1 */
	CPU_AHB2APB_DIV_SHIFT	= 14,
	CPU_AHB2APB_DIV_MASK	= 3,
	 ...
	 */
	PERI_ACLK_SEL_PLL_SHIFT	= 15,
	PERI_ACLK_SEL_PLL_MASK	= 1,
	PERI_ACLK_SEL_CPLL	= 0,
	PERI_ACLK_SEL_GPLL,
...
static void rkclk_init(struct rk3188_cru *cru, struct rk3188_grf *grf)
{
	...
	/*
	 * cpu-domain clock pll source selection and
	 * set up dependent divisors for PCLK/HCLK and ACLK clocks.
	 */
	aclk_div = GPLL_HZ / CPU_ACLK_HZ - 1;
	assert((aclk_div + 1) * CPU_ACLK_HZ == GPLL_HZ && aclk_div < 0x1f);

	hclk_div = log2(CPU_ACLK_HZ / CPU_HCLK_HZ);
	assert((1 << hclk_div) * CPU_HCLK_HZ ==
		CPU_ACLK_HZ && (hclk_div < 0x3));

	pclk_div = log2(CPU_ACLK_HZ / CPU_PCLK_HZ);
	assert((1 << pclk_div) * CPU_PCLK_HZ ==
		CPU_ACLK_HZ && (pclk_div < 0x4));

	/* set the ahb2apb pclk rate to the same as the cpu pclk */
	ahb2apb_div = log2(CPU_HCLK_HZ / CPU_PCLK_HZ);
	assert((1 << ahb2apb_div) * CPU_PCLK_HZ ==
		CPU_HCLK_HZ && (ahb2apb_div < 0x3));

	rk_clrsetreg(&cru->cru_clksel_con[0],
		     CPU_ACLK_SEL_PLL_MASK << CPU_ACLK_SEL_PLL_SHIFT |
		     CPU_ACLK_DIV_MASK << CPU_ACLK_DIV_SHIFT,
		     CPU_ACLK_SEL_GPLL << CPU_ACLK_SEL_PLL_SHIFT |
		     aclk_div << CPU_ACLK_DIV_SHIFT
		     );

	rk_clrsetreg(&cru->cru_clksel_con[1],
		     CPU_PCLK_DIV_MASK << CPU_PCLK_DIV_SHIFT |
		     CPU_HCLK_DIV_MASK << CPU_HCLK_DIV_SHIFT |
		     CPU_AHB2APB_DIV_MASK << CPU_AHB2APB_DIV_SHIFT,
		     pclk_div << CPU_PCLK_DIV_SHIFT |
		     hclk_div << CPU_HCLK_DIV_SHIFT |
		     ahb2apb_div << CPU_AHB2APB_DIV_SHIFT
		     );
	...
	rk_clrsetreg(&cru->cru_clksel_con[10],
		     PERI_ACLK_SEL_PLL_MASK << PERI_ACLK_SEL_PLL_SHIFT |
		     PERI_PCLK_DIV_MASK << PERI_PCLK_DIV_SHIFT |
		     PERI_HCLK_DIV_MASK << PERI_HCLK_DIV_SHIFT |
		     PERI_ACLK_DIV_MASK << PERI_ACLK_DIV_SHIFT,
		     PERI_ACLK_SEL_GPLL << PERI_ACLK_SEL_PLL_SHIFT |
...
static ulong rk3188_clk_get_rate(struct udevice *dev)
{
...
static ulong rk3188_clk_set_rate(struct udevice *dev, ulong rate)
{
	struct rk3188_clk_plat *plat = dev_get_platdata(dev);
	struct rk3188_clk_priv *priv = dev_get_priv(dev);
...
/* FIXME: mmc clocks are not source from plls but from hclk_peri instead */
static ulong rockchip_mmc_get_clk(struct rk3188_cru *cru, uint clk_general_rate,
...
static ulong rockchip_mmc_set_clk(struct rk3188_cru *cru, uint clk_general_rate,
...
/*static ulong rockchip_spi_get_clk(struct rk3188_cru *cru, uint clk_general_rate,
				  enum periph_id periph)
{
	uint div, mux;
	u32 con;
...
/*static ulong rockchip_spi_set_clk(struct rk3188_cru *cru, uint clk_general_rate,
				  enum periph_id periph, uint freq)
{
	int src_clk_div;
...
ulong rk3188_set_periph_rate(struct udevice *dev, int periph, ulong rate)
{
	struct rk3188_clk_priv *priv = dev_get_priv(dev);
	ulong new_rate;
...
static struct clk_ops rk3188_clk_ops = {
	...
	.set_periph_rate = rk3188_set_periph_rate,
};
...
static int rk3188_clk_probe(struct udevice *dev)
{
	...
	if (plat->clk_id != CLK_OSC) {
		struct rk3188_clk_priv *parent_priv = dev_get_priv(dev->parent);
...
static int rk3188_clk_bind(struct udevice *dev)
{
	...
	for (pll = 1; pll < CLK_COUNT; pll++) {
		...
		struct rk3188_clk_plat *cplat;

		debug("%s %s\n", __func__, clk_name[pll]);
		ret = device_bind_driver(dev, "clk_rk3188", clk_name[pll],
					 &child);
		if (ret)
			return ret;
	...
	ret = device_bind_driver(gd->dm_root, "rk3188_reset", "reset", &dev);
	if (ret)
